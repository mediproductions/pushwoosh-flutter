...
class Pushwoosh {
  ...
  static const EventChannel _receiveChannel =
      const EventChannel('pushwoosh/receive');
  static const EventChannel _acceptChannel =
      const EventChannel('pushwoosh/accept');
  static const EventChannel _openChannel =
      const EventChannel('pushwoosh/deeplink');
  ...
  Future<bool?> get showForegroundAlert async =>
      await _channel.invokeMethod("showForegroundAlert");
  ...
  Stream<PushEvent> get onPushReceived =>
      _receiveChannel.receiveBroadcastStream().map((dynamic event) =>
          _toPushwooshMessage(event.cast<dynamic, dynamic>()));

  /// Get the [Stream] of accepted [PushwooshMessage].
  Stream<PushEvent> get onPushAccepted =>
      _acceptChannel.receiveBroadcastStream().map((dynamic event) =>
          _toPushwooshMessage(event.cast<dynamic, dynamic>()));
  ...
  PushEvent _toPushwooshMessage(Map<dynamic, dynamic> map) {
    var pushwooshMessage = new PushwooshMessage(
        map['title'], map['message'], map['payload'], map['customData']);
  ...
  Future<String?> get getPushToken async =>
      await _channel.invokeMethod("getPushToken");
